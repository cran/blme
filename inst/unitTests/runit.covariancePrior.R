cat("\n\nRUnit test cases for blme:::parsePrior function, cov.prior argument\n\n");

# generated by:
if (FALSE) {
  set.seed(0);
  N <- 50;
  J.1 <- 5;
  J.2 <- 5;
  beta <- c(5, 2, 4);
  theta.1 <- matrix(rnorm(J.1 * 2), J.1, 2);
  theta.2 <- matrix(rnorm(J.2 * 3), J.2, 3);
  
  x.1 <- rnorm(N);
  x.2 <- rnorm(N);
  g.1 <- rmultinom(N, 1, runif(J.1));
  g.2 <- rmultinom(N, 1, runif(J.2));
  g.1 <- sapply(1:N, function(i) which(g.1[,i] == 1));
  g.2 <- sapply(1:N, function(i) which(g.2[,i] == 1));

  y <- rep(NA, N);
  for (i in 1:N) {
    y[i] <- 1 * (beta[1] + theta.1[g.1[i], 1] + theta.2[g.2[i], 1]) +
       x.1[i] * (beta[2] + theta.1[g.1[i], 2] + theta.2[g.2[i], 2]) +
       x.2[i] * (beta[3] +                      theta.2[g.2[i], 3]) +
       rnorm(1);
  }
}

test.blme.parsePrior.cov.prior <- function()
{
  x.1 <- c(0.503607972233726, 1.08576936214569, -0.69095383969683, -1.28459935387219, 0.046726172188352, -0.235706556439501, -0.542888255010254, -0.433310317456782, -0.649471646796233, 0.726750747385451, 1.1519117540872, 0.992160365445798, -0.429513109491881, 1.23830410085338, -0.279346281854269, 1.75790308981071, 0.560746090888056, -0.452783972553158, -0.832043296117832, -1.16657054708471, -1.0655905803883, -1.563782051071, 1.15653699715018, 0.83204712857239, -0.227328691424755, 0.266137361672105, -0.376702718583628, 2.44136462889459, -0.795339117255372, -0.0548774737115786, 0.250141322854153, 0.618243293566247, -0.172623502645857, -2.22390027400994, -1.26361438497058, 0.358728895971352, -0.0110454784656636, -0.940649162618608, -0.115825322156954, -0.814968708869917, 0.242263480859686, -1.4250983947325, 0.36594112304922, 0.248412648872596, 0.0652881816716207, 0.0191563916602738, 0.257338377155533, -0.649010077708898, -0.119168762418038, 0.66413569989411);

  x.2 <- c(1.10096910219409, 0.14377148075807, -0.117753598165951, -0.912068366948338, -1.43758624082998, -0.797089525071965, 1.25408310644997, 0.77214218580453, -0.21951562675344, -0.424810283377287, -0.418980099421959, 0.996986860909106, -0.275778029088027, 1.2560188173061, 0.646674390495345, 1.29931230256343, -0.873262111744435, 0.00837095999603331, -0.880871723252545, 0.59625901661066, 0.119717641289537, -0.282173877322451, 1.45598840106634, 0.229019590694692, 0.996543928544126, 0.781859184600258, -0.776776621764597, -0.615989907707918, 0.0465803028049967, -1.13038577760069, 0.576718781896486, -1.28074943178832, 1.62544730346494, -0.500696596002705, 1.67829720781629, -0.412519887482398, -0.97228683550556, 0.0253828675878054, 0.0274753367451927, -1.68018272239593, 1.05375086302862, -1.11959910457218, 0.335617209968815, 0.494795767113158, 0.138052708711737, -0.118792025778828, 0.197684262345795, -1.06869271125479, -0.80321321736474, -1.11376513631953);

  y <- c(10.2556670398245, 5.77156184409445, 5.2388963179889, -1.01048113754769, -2.40694356326161, 4.87659537730766, 11.0678751360507, 9.95236105245466, -1.21089148312542, 5.80498258814754, 0.684860586400015, 11.0298099678496, 5.71308913175892, 16.7456124781686, 7.4731450675828, 8.51093715973169, 2.82372959757841, 4.3443694315618, -2.96487863376567, 5.88586538048499, 0.976211065859574, 5.62948932886417, 13.6141669016732, 6.07894540274009, 9.57669016288729, 8.56529931086956, 2.02623726516967, 5.335419611075, 5.49337753963837, 0.357086593483832, 5.90421996841909, -0.0078591135781455, 12.5466815499627, -6.66182740887203, 11.9526738840087, 1.5017901567396, 1.15948360785528, 4.97438059345444, 5.91648438771629, 0.262312751711231, 10.5968851648003, -0.281646718883028, 7.38808913061462, 7.91783952393784, 6.24106797266484, 4.86354361177658, 5.04480479030089, 3.28778785631259, 1.36283750127861, -1.0792461964562);

  g.1 <- c(5, 2, 4, 4, 5, 5, 4, 3, 5, 5, 2, 3, 4, 5, 2, 2, 5, 1, 5, 2, 5, 1, 1, 1, 1, 1, 2, 1, 3, 5, 2, 1, 4, 5, 4, 2, 2, 2, 4, 3, 3, 3, 1, 5, 1, 4, 2, 3, 1, 2);
  g.2 <- c(3, 4, 1, 2, 4, 1, 1, 4, 3, 3, 3, 3, 1, 4, 3, 3, 2, 3, 3, 2, 4, 3, 4, 3, 3, 1, 3, 4, 3, 3, 3, 2, 3, 3, 2, 3, 3, 3, 4, 1, 4, 3, 3, 3, 4, 5, 2, 1, 3, 3);

  options(warn = -1);
  model1 <- blmer(y ~ x.1 + (1 | g.1), control = list(maxIter = 0L),
                  cov.prior = NULL, fixef.prior = NULL);
  options(warn = 0);
  
  prior <- blme:::parsePrior(model1, cov.prior = "gamma");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 1);
    checkEquals(length(prior[[1]]@hyperparameters), 2);
    checkEquals(prior[[1]]@families[1], blme:::getEnumOrder(blme:::familyEnumeration, "gamma"));
  }

  prior <- blme:::parsePrior(model1, cov.prior = "gamma(3)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 1);
    checkEquals(length(prior[[1]]@hyperparameters), 2);
    checkEquals(prior[[1]]@families[1], blme:::getEnumOrder(blme:::familyEnumeration, "gamma"));
    checkEquals(prior[[1]]@hyperparameters[1], 3);
  }
  
  prior <- blme:::parsePrior(model1, cov.prior = "gamma(2, 1.8)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 1);
    checkEquals(length(prior[[1]]@hyperparameters), 2);
    checkEquals(prior[[1]]@families[1], blme:::getEnumOrder(blme:::familyEnumeration, "gamma"));
    checkEquals(prior[[1]]@hyperparameters, c(2, 1.8));
  }
  
  prior <- blme:::parsePrior(model1, cov.prior = "gamma(rate = 1.8, shape = 2, posterior.scale = 'var')")
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 1);
    checkEquals(length(prior[[1]]@hyperparameters), 2);
    checkEquals(prior[[1]]@families[1], blme:::getEnumOrder(blme:::familyEnumeration, "gamma"));
    checkEquals(prior[[1]]@scales[1], blme:::getEnumOrder(blme:::scaleEnumeration, "var"))
    checkEquals(prior[[1]]@hyperparameters, c(2, 1.8));
  }
  
  options(warn = -1);
  model2 <- blmer(y ~ x.1 + x.2 + (1 + x.1 | g.1) + (1 + x.1 + x.2 | g.2),
                  control = list(maxIter = 0L),
                  cov.prior = NULL, fixef.prior = NULL);
  options(warn = 0);
  d.1 <- 2;
  d.2 <- 3;
  prior <- blme:::parsePrior(model2, cov.prior = "g.1 ~ spectral, g.2 ~ none");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "none"));
    checkEquals(length(prior[[1]]@families), d.1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1);
    checkEquals(length(prior[[2]]@families), 0);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 0);
  }

  prior <- blme:::parsePrior(model2, cov.prior = "g.1 ~ spectral(inverse.gamma), g.2 ~ none");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "none"));
    checkEquals(length(prior[[1]]@families), d.1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1);
    checkEquals(length(prior[[2]]@families), 0);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 0);

    checkEquals(prior[[1]]@families,
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "inverse.gamma"), d.1));
  }

  prior <- blme:::parsePrior(model2, cov.prior = "g.1 ~ spectral(gamma(shape = 3, rate = .25)), g.2 ~ none");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "none"));
    checkEquals(length(prior[[1]]@families), d.1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1);
    checkEquals(length(prior[[2]]@families), 0);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 0);

    checkEquals(prior[[1]]@families,
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.1));
    checkEquals(prior[[1]]@hyperparameters, rep(c(3, 0.25), d.1));
  }

  prior <- blme:::parsePrior(model2, cov.prior = "g.1 ~ spectral(gamma(shape = 3, rate = .25, posterior.scale = 'sd')), g.2 ~ none");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "none"));
    checkEquals(length(prior[[1]]@families), d.1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1);
    checkEquals(length(prior[[2]]@families), 0);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 0);

    checkEquals(prior[[1]]@families,
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.1));
    checkEquals(prior[[1]]@hyperparameters, rep(c(3, 0.25), d.1));
    checkEquals(prior[[1]]@scales,
                rep(blme:::getEnumOrder(blme:::scaleEnumeration, "sd"), d.1));
  }
  
  prior <- blme:::parsePrior(model2, cov.prior = "spectral(gamma(shape = 3, rate = .25, posterior.scale = 'sd'))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(length(prior[[1]]@families), d.1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1);
    checkEquals(length(prior[[2]]@families), d.2);
    checkEquals(length(prior[[2]]@scales), d.2);
    checkEquals(length(prior[[2]]@hyperparameters), 2 * d.2);

    
    checkEquals(prior[[1]]@families,
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.1));
    checkEquals(prior[[1]]@hyperparameters, rep(c(3, 0.25), d.1));
    checkEquals(prior[[1]]@scales,
                rep(blme:::getEnumOrder(blme:::scaleEnumeration, "sd"), d.1));
                
    checkEquals(prior[[2]]@families,
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.2));
    checkEquals(prior[[2]]@hyperparameters, rep(c(3, 0.25), d.2));
    checkEquals(prior[[2]]@scales,
                rep(blme:::getEnumOrder(blme:::scaleEnumeration, "sd"), d.2));
  }

  prior <- blme:::parsePrior(model2, cov.prior = "correlation(gamma, inverse.wishart)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "correlation"));
    checkEquals(length(prior[[1]]@families), d.1 + 1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1 + 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "correlation"));
    checkEquals(length(prior[[2]]@families), d.2 + 1);
    checkEquals(length(prior[[2]]@scales), d.2);
    checkEquals(length(prior[[2]]@hyperparameters), 2 * d.2 + 2 + d.2^2);

    checkEquals(prior[[1]]@families[1:d.1],
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.1));
    checkEquals(prior[[1]]@families[d.1 + 1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    
    checkEquals(prior[[2]]@families[1:d.2],
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.2));
    checkEquals(prior[[2]]@families[d.2 + 1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
  }
  
  prior <- blme:::parsePrior(model2, cov.prior =
                      "g.1 ~ correlation(inverse.gamma, wishart), g.2 ~ spectral(gamma)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "correlation"));
    checkEquals(length(prior[[1]]@families), d.1 + 1);
    checkEquals(length(prior[[1]]@scales), d.1);
    checkEquals(length(prior[[1]]@hyperparameters), 2 * d.1 + 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "spectral"));
    checkEquals(length(prior[[2]]@families), d.2);
    checkEquals(length(prior[[2]]@scales), d.2);
    checkEquals(length(prior[[2]]@hyperparameters), 2 * d.2);
                
    checkEquals(prior[[1]]@families[1:d.1],
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "inverse.gamma"), d.1));
    checkEquals(prior[[1]]@families[d.1 + 1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    
    checkEquals(prior[[2]]@families[1:d.2],
                rep(blme:::getEnumOrder(blme:::familyEnumeration, "gamma"), d.2));
  }
  
  prior <- blme:::parsePrior(model2, cov.prior = "g.1 ~ wishart, g.2 ~ inverse.wishart");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + d.2^2);

    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
  }

  prior <- blme:::parsePrior(model2, cov.prior = "wishart");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + d.2^2);

    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
  }
  
  prior <- blme:::parsePrior(model2, cov.prior = "wishart(df = expression(factorDimension + 4))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + d.2^2);

    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[1], d.1 + 4);
    
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters[1], d.2 + 4);
  }

  prior <- blme:::parsePrior(model2, cov.prior = "wishart(scale = expression(diag(dataSd^2, factorDimension)))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + d.1^2);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + d.2^2);

    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[1:(d.1^2) + 2], as.numeric(diag(1 / var(y), d.1)));
    
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters[1:(d.2^2) + 2], as.numeric(diag(1 / var(y), d.2)));
  }

  options(warn = -1);
  model3 <- blmer(y ~ x.1 + (1 + x.1 | g.1) + (1 | g.2),
                  control = list(maxIter = 0L),
                  cov.prior = NULL, fixef.prior = NULL);
  options(warn = 0);
  d.1 <- 2;
  d.2 <- 1;
  
  prior <- blme:::parsePrior(model3, cov.prior = "inverse.gamma, wishart");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 1);
    checkEquals(length(prior[[2]]@hyperparameters), 2);

    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.gamma"));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "wishart(df = expression(factorDimension + 1))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[1], 3);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters[1], 2);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "wishart(scale = 0.01)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(100.0, 2)));
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters[3], 100.0);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "wishart(scale = expression(diag(0.01, factorDimension)))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(100.0, 2)));
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters[3], 100.0);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ wishart");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ wishart(df = 3)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[1], 3);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ wishart(scale = 0.01)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(100.0, 2)));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ wishart(scale = diag(0.01, 2))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(100.0, 2)));
  }
  
  
  prior <- blme:::parsePrior(model3, cov.prior = "inverse.wishart(df = expression(factorDimension + 1))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[1]]@hyperparameters[1], 3);
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[2]]@hyperparameters[1], 2);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "inverse.wishart(inverse.scale = 0.01)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(0.01, 2)));
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[2]]@hyperparameters[3], 0.01);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "inverse.wishart(inverse.scale = expression(diag(0.01, factorDimension)))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(0.01, 2)));
    
    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + 1);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[2]]@hyperparameters[3], 0.01);
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ inverse.wishart");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ inverse.wishart(df = 3)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@hyperparameters[1], 3);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ inverse.wishart(inverse.scale = 0.01)");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(0.01, 2)));
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
  }
  
  prior <- blme:::parsePrior(model3, cov.prior = "g.1 ~ inverse.wishart(inverse.scale = diag(0.01, 2))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + 4);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "inverse.wishart"));
    checkEquals(prior[[1]]@hyperparameters[3:6], as.numeric(diag(0.01, 2)));
  }

  prior <- blme:::parsePrior(model3, cov.prior = "wishart(data.scale = 'free', scale = 1, df = expression(factorDimension + 3))");
  {
    checkEquals(prior[[1]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[1]]@families), 1);
    checkEquals(length(prior[[1]]@scales), 0);
    checkEquals(length(prior[[1]]@hyperparameters), 2 + d.1^2);
    checkEquals(prior[[1]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    testCovar <- diag(c(var(y), var(y) / var(x.1)), 2);
    checkEquals(prior[[1]]@hyperparameters,
                c(d.1 + 3, determinant(testCovar, logarithm="true")$modulus,
                  as.numeric(solve(testCovar))));

    checkEquals(prior[[2]]@type, blme:::getEnumOrder(blme:::typeEnumeration, "direct"));
    checkEquals(length(prior[[2]]@families), 1);
    checkEquals(length(prior[[2]]@scales), 0);
    checkEquals(length(prior[[2]]@hyperparameters), 2 + d.2^2);
    checkEquals(prior[[2]]@families[1],
                blme:::getEnumOrder(blme:::familyEnumeration, "wishart"));
    checkEquals(prior[[2]]@hyperparameters,
                c(d.2 + 3, log(var(y)), 1 / var(y)));
  }
}
